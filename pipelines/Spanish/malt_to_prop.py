    ...
    for line in infile:
        ...
        if len(line) > 1:
            ...
            for i in range(len(props)):
                if not propTags.match(props[0][2]):
                    propID = "["+str(sent_count)+str(props[0][0])+"]"
                    sys.stdout.write(propID+":"+props[0][1])
                    props.pop(0)
                elif propTags.match(props[0][2]):
                    simple_predicate,eCount,xCount = build_predicate(props[0][2],eCount,xCount)
                    predicate = "("+",".join(simple_predicate)+")"
                    propID = "["+str(sent_count)+str(props[0][0])+"]"
                    sys.stdout.write(propID+":"+props[0][1]+"-"+props[0][2]+predicate)
                    props.pop(0)
